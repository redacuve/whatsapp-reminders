...
export async function handleCommand(
  ...
  // Remi about command
  if (lower === 'remi' || lower === 'about' || lower === 'about remi') {
    const name = ctx.senderPushName || ctx.senderName;
    await send(locale.responses.remiAbout(name));
    return;
  }

  // Greeting
  if (locale.responses.greetingKeywords.includes(lower)) {
    const name = ctx.senderPushName || ctx.senderName;
    const hour = ctx.timestamp.getHours();
    await send(
      pickRandom(locale.responses.greetings[getTimeOfDay(hour)])(name),
    );
    return;
  }

  // Farewell
  if (locale.responses.farewellKeywords.includes(lower)) {
    await send(pickRandom(locale.responses.farewells));
    return;
  }

  // Thanks
  if (locale.responses.thanksKeywords.some((k) => lower.includes(k))) {
    await send(pickRandom(locale.responses.thanks));
    return;
  }

  // How are you
  if (locale.responses.howAreYouKeywords.some((k) => lower.includes(k))) {
    await send(pickRandom(locale.responses.howAreYou));
    return;
  }

  // Emoji reaction
  if (locale.responses.emojiKeywords.includes(trimmed)) {
    await send(pickRandom(locale.responses.emojiReactions));
    return;
  }

  // Who are you
  if (locale.responses.whoAreYouKeywords.some((k) => lower.includes(k))) {
    await send(pickRandom(locale.responses.whoAreYou));
    return;
  }

  // What can you do
  if (locale.responses.whatCanYouDoKeywords.some((k) => lower.includes(k))) {
    await send(pickRandom(locale.responses.whatCanYouDo));
    return;
  }

  // Laughter
  if (
    locale.responses.laughKeywords.some((k) => lower === k || trimmed === k)
  ) {
    await send(pickRandom(locale.responses.laughResponses));
    return;
  }

  // Compliment
  if (locale.responses.complimentKeywords.some((k) => lower.includes(k))) {
    await send(pickRandom(locale.responses.complimentResponses));
    return;
  }

  // Sorry / apologies
  if (locale.responses.sorryKeywords.some((k) => lower.includes(k))) {
    await send(pickRandom(locale.responses.sorryResponses));
    return;
  }
